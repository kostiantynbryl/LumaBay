...
namespace LumaBay
{
    public sealed partial class LumaBayGame
    {
        ...
        private void ApplyBoosterArtwork(Transform root)
        {
            ...
            {
                if (!item.name.StartsWith("Booster_", StringComparison.Ordinal) ||
                    item.Find("PremiumBoosterIcon") != null)
                    continue;

                Text[] labels = item.GetComponentsInChildren<Text>(true);
                string id = ResolveBoosterId(item.name, labels);
                if (string.IsNullOrEmpty(id)) continue;

                Sprite sprite = LumaBayArtPackV2.Booster(id) ?? LumaBayArtPack.Booster(id);
                if (sprite == null) continue;

                Image icon = CreateImage(item, "PremiumBoosterIcon", sprite, Color.white);
                icon.rectTransform.anchorMin = new Vector2(0.16f, 0.28f);
                icon.rectTransform.anchorMax = new Vector2(0.84f, 0.90f);
                icon.rectTransform.offsetMin = Vector2.zero;
                icon.rectTransform.offsetMax = Vector2.zero;
                icon.raycastTarget = false;
                LayoutElement ignore = icon.gameObject.AddComponent<LayoutElement>();
        ...
        private void ApplyLighthouseBeam(Transform root)
        {
            ...
            foreach (Transform item in transforms)
            {
                if (item.name != "PremiumLighthouseVisual" || item.Find("AnimatedBeam") != null) continue;

                RectTransform beam = CreateRect(item, "AnimatedBeam");
    ...
    public sealed class LighthouseIllustrationMotion : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            rect.localScale = baseScale * (1f + Mathf.Sin(time * 0.35f + phase) * 0.0025f);
        }
    }
    ...
    public sealed class LighthouseBeamSweep : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            color.a = Mathf.Lerp(0.12f, 0.31f,
                (Mathf.Sin(Time.unscaledTime * 0.72f + phase) + 1f) * 0.5f);
            graphic.color = color;
        }
    }
}
